    ...
    public async handle(message: Message): Promise<any> {
        ...
        if (timestamps?.has(message.author.id)) {
            ...
            if (now < expirationTime) {
                ...
                message.channel.send({ embeds: [createEmbed("error", `${message.author.toString()}, please wait **\`${timeLeft.toFixed(1)}\`** of cooldown time.`, true)] }).then(m => {
                    setTimeout(() => m.delete().catch(e => this.client.logger.error("PROMISE_ERR:", e)), 3500);
